...
public class FluentAPIWithManyValuedFeatTest {
	...
	public void apiTest_WithManyRemovedValuedEAttribute_AsArray() {
		...
		var finalNss = new String[] { "ns1", "ns3" };

		var mod = api.newModule().withAddedNamespaces(modNss).withRemovedNamespaces(nsToRemove).createNow();
		Assertions.assertInstanceOf(org.emftext.language.java.containers.Module.class, mod);
		Assertions.assertEquals(finalNss.length, mod.getNamespaces().size());
		Assertions.assertFalse(mod.getNamespaces().retainAll(List.of(finalNss)));
	}
	...
	public void apiTest_WithManyRemovedValuedEReference_AsArray() {
		...
		var pacs = new org.emftext.language.java.containers.Package[] { pac1, pacToRemove, pac3 };
		var finalPacs = new org.emftext.language.java.containers.Package[] { pac1, pac3 };

		var mod = api.newModule().withAddedPackages(pacs).withRemovedPackages(pacToRemove).createNow();
		Assertions.assertInstanceOf(org.emftext.language.java.containers.Module.class, mod);
		Assertions.assertEquals(finalPacs.length, mod.getPackages().size());
		Assertions.assertFalse(mod.getPackages().retainAll(List.of(finalPacs)));
	}
