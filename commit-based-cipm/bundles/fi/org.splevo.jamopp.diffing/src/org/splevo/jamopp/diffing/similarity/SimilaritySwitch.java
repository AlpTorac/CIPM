...
import org.splevo.jamopp.diffing.similarity.switches.ISimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.annotations.AnnotationSettingsSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.annotations.AnnotationsSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.classifiers.AnonymousClassSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.classifiers.ConcreteClassifiersSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.containers.CompilationUnitSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.containers.ModuleSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.containers.PackageSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.CatchBlockSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.ConditionalsSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.ExpressionStatementsSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.JumpLabelsSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.JumpSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.LocalVariableStatementsSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.ReturnStatementsSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.SwitchesSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.SynchronizedBlocksSimilaritySwitch;
import org.splevo.jamopp.diffing.similarity.switches.statements.ThrowsSimilaritySwitch;
...
public class SimilaritySwitch {
	...
	private Collection<ISimilaritySwitch> sSwitches;
     ...
     */
    public SimilaritySwitch(boolean checkStatementPosition,
        ...
        this.similarityChecker = new SimilarityChecker(this.classifierNormalizations, this.compilationUnitNormalizations,
                packageNormalizations);
        this.similarityChecker.setCheckStatementPositionOnDefault(checkStatementPosition);
        this.sSwitches = this.initSwitches();
...
//        SSLoggingHelper.setLogger(fiLogger);
    ...
    protected Collection<ISimilaritySwitch> initSwitches() {
    	var csp = this.similarityChecker.checksStatementPositionOnDefault();
    	var result = new ArrayList<ISimilaritySwitch>();
    	
    	// Annotations
    	result.add(new AnnotationsSimilaritySwitch());
    	result.add(new AnnotationSettingsSimilaritySwitch());
    	
    	// Classifiers
    	result.add(new ConcreteClassifiersSimilaritySwitch(this.classifierNormalizations));
    	result.add(new AnonymousClassSimilaritySwitch());
    	
    	// Commons
    	result.add(new CommonsSimilaritySwitch());
    	
    	// Containers
    	result.add(new CompilationUnitSimilaritySwitch(this.compilationUnitNormalizations,
    			this.packageNormalizations));
    	result.add(new PackageSimilaritySwitch(this.packageNormalizations));
    	result.add(new ModuleSimilaritySwitch());
    	
    	// Statements
    	result.add(new CatchBlockSimilaritySwitch(csp));
    	result.add(new ConditionalsSimilaritySwitch(csp));
    	result.add(new ExpressionStatementsSimilaritySwitch(csp));
    	result.add(new JumpLabelsSimilaritySwitch(csp));
    	result.add(new JumpSimilaritySwitch(csp));
    	result.add(new LocalVariableStatementsSimilaritySwitch(csp));
    	result.add(new ReturnStatementsSimilaritySwitch(csp));
    	result.add(new SwitchesSimilaritySwitch(csp));
    	result.add(new SynchronizedBlocksSimilaritySwitch(csp));
    	result.add(new ThrowsSimilaritySwitch(csp));
    	
    	return result;
    }
    
    public Boolean compare(EObject eo1, EObject eo2, SimilarityChecker sc) {
    	for (var ss : this.sSwitches) {
    		if (ss.canHandle(eo1, eo2)) {
    			return ss.handle(eo1, eo2, sc);
    		}
    	}
...
//         * <p>
//         * All expression elements are strong typed with no identifying attributes or noncontainment
...
//         * <p>
//         * All modifier elements are strong typed with no identifying attributes or noncontainment
//         * references. Their location and runtime types are assumed to be checked before this switch is
//         * called.
//         * </p>
//         */
...
//         * <p>
//         * All operator elements are strong typed with no identifying attributes or noncontainment
//         * references. Their location and runtime types are assumed to be checked before this switch is
//         * called.
//         * </p>
//         */
...
//             * 
//             * @param varStmt1
//             *            The variable to compare with the original / rightside one
...
//    +    	result.add(new JumpLabelsSimilaritySwitch(csp));
//    +    	result.add(new JumpSimilaritySwitch(csp));
//    +    	result.add(new LocalVariableSimilaritySwitch(csp));
//    +    	return result;
//         }
//         
//        /**
