...
public class HierarchicalMatchEngine implements IMatchEngine {
    ...
    public HierarchicalMatchEngine(IEqualityHelper equalityHelper, EqualityStrategy equalityStrategy,
        ...
        fiLogger.debug("HierarchicalMatchEngine instantiated");
    }

    @Override
    public Comparison match(IComparisonScope scope, Monitor monitor) {
    	fiLogger.debug("Matching with given scope and monitor");
    ...
    protected void match(Comparison comparison, IComparisonScope scope, final Notifier left, final Notifier right,
            Monitor monitor) {
    	fiLogger.debug("Delegating to proper match function, checking types");
        if (left instanceof ResourceSet || right instanceof ResourceSet) {
        	fiLogger.debug("ResourceSet found. Is left/right ResourceSet: " +
        	...
        	fiLogger.debug("Resource found. Is left/right Resource: " +
        	...
        	fiLogger.debug("EObject found. Is left/right EObject: " +
        			(left instanceof EObject) + "/" + (right instanceof EObject));
            match(comparison, (EObject) left, (EObject) right, monitor);
        } else {
        	fiLogger.debug("Unhandled type of elements to match. (" + left + ", " + right + ")");
    ...
    protected void match(Comparison comparison, IComparisonScope scope, ResourceSet left, ResourceSet right,
            Monitor monitor) {
    	fiLogger.debug("Matching given ResourceSet instances");
        ...
        fiLogger.debug("Creating resource matcher and mappings");
        ...
        fiLogger.debug("Adding matched resources to comparison");
        for (MatchResource mapping : mappings) {
            ...
            fiLogger.debug("Matching: " + leftRes + " with " + rightRes);
    ...
    protected void match(Comparison comparison, final Resource leftRes, final Resource rightRes, Monitor monitor) {
    	fiLogger.debug("Matching given Resource instances");
        ...
        if (leftRes != null) {
        	fiLogger.debug("Left resource not null, getting contents");
            leftElements = leftRes.getContents();
        }
        if (rightRes != null) {
        	fiLogger.debug("Right resource not null, getting contents");
            rightElements = rightRes.getContents();
        }

        List<Match> matches = match(comparison, leftElements, rightElements, monitor);
        fiLogger.debug("Adding all found matches");
    ...
    protected void match(Comparison comparison, final EObject left, final EObject right, Monitor monitor) {
    	fiLogger.debug("Matching given EObject instances");
    ...
    private List<Match> match(Comparison comparison, List<EObject> leftElements, List<EObject> rightElements,
            Monitor monitor) {
    	fiLogger.debug("Matching given EObject instance elements recursively");
        ...
        for (EObject leftElement : leftElementsInScope) {
            ...
            for (EObject rightElement : rightElementsInScope) {
                if (equalityStrategy.areEqual(leftElement, rightElement)) {
                	fiLogger.debug("EObject elements matched, recursing function call");
            ...
            if (match.getRight() == null) {
            	fiLogger.debug("Right element is null, substituting with empty list and recursing");
        ...
        fiLogger.debug("Elements in left scope matched");
    ...
    private List<Match> createMatchesForRightElements(List<EObject> elements) {
    	fiLogger.debug("Creating right only matches recursively");
