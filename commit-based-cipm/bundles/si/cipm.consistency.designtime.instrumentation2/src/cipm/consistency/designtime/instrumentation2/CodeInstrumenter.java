...
public final class CodeInstrumenter {
	...
	public static <CM extends ModelFacade> Resource instrument(CommitIntegrationState<CM> state, boolean adaptive) {
		...
		ResourceSet codeModelResourceSet = state.getCodeModelFacade().getResourceSet();
		Resource codeModel = codeModelResourceSet.getResources().get(0);

		ResourceSet targetSet = new ResourceSetImpl();
		Resource copy = targetSet.createResource(codeModel.getURI());
		copy.getContents().addAll(EcoreUtil.copyAll(codeModel.getContents()));
		...
		for (ServiceInstrumentationPoint sip : im.getPoints()) {
			LOGGER.trace("Instrumenting the service " + sip.getService().getDescribedService__SEFF().getEntityName());
...
//                .next();
			...
			var corrSet = state.getVsumFacade().getCorrespondenceView().getCorrespondingEObjects(sip);
			if (corrSet.size() != 1) {
				throw new IllegalStateException("Multiple code model elements correspond to a SIP");
			}

			EObject codeModelServiceElement = corrSet.iterator().next();
			EObject copiedService = findCopiedEObject(targetSet, codeModelServiceElement);

			ActionStatementMapping statementMap = createActionStatementMapping(targetSet, sip,
					state.getVsumFacade().getCorrespondenceView());
		...
		var path = state.getDirLayout().getRootDirPath().resolve("instrumented-code");
		ModelSaverInRepositoryCopy.saveModels(targetSet, copy, path,
				state.getGitRepositoryWrapper().getWorkTree().toPath(), gen);
