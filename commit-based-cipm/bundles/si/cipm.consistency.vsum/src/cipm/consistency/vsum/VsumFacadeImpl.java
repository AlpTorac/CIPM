...
//            .forEach(ele -> viewSelector.setSelected(ele, true));
		...
		var view = viewSelector.createView().withChangeDerivingTrait(stateBasedChangeResolutionStrategy);
	...
	public CommittableView getChangeRecordingView() {
		...
		var view = viewSelector.createView().withChangeRecordingTrait();
...
//		pcmDomain.enableTransitiveChangePropagation();
	 ...
	 * @param resource  The propagated resource
	 * @param targetUri The uri where vitruv persists the propagated resource
	 * @param vsum      Optional, may be used to override the vsum to which the
