...
public class StatementPositionTest extends AbstractJaMoPPSimilarityTest implements IStatementTest {
	...
	private static Stream<Arguments> genTestParams() {
		...
		for (var stCls : JaMoPPArguments.getAllConcreteClassesBySuper(Statement.class)) {
			for (var slcCls : JaMoPPArguments.getAllConcreteClassesBySuper(StatementListContainer.class)) {
	 ...
	 * @param containerCls The initialiser that will be used to instantiate the
	 *                     statement list container
	 * @param containeeCls The initialiser that will be used to instantiate the
	 *                     statement that will be added to the constructed container
	 */
	@ParameterizedTest(name = "Statement in middle: {2}")
	@MethodSource("genTestParams")
	public void test_SamePredecessor_SameSuccessor(Class<? extends Statement> containeeCls,
			Class<? extends StatementListContainer> containerCls, String displayName) {
		var slc1 = getAPI().createNewX(containerCls);
		var slc2 = getAPI().createNewX(containerCls);

		// Use 3 statements to make sure that there is always a predecessor and a
		// successor for the statement generated by containeeCls

		var pred1 = getAPI().newLocalVariableStatement(getAPI().newLocalVariable().withName("lv1").createNow());
		var st1 = getAPI().createNewX(containeeCls);
		var succ1 = getAPI().newLocalVariableStatement(getAPI().newLocalVariable().withName("lv2").createNow());
		...
		Assertions.assertTrue(addStatements(slc1, sts1));
		Assertions.assertTrue(addStatements(slc2, sts2));
	...
	@ParameterizedTest(name = "Statement in middle: {2}")
	@MethodSource("genTestParams")
	public void test_DifferentPredecessor_SameSuccessor(Class<? extends Statement> containeeCls,
			Class<? extends StatementListContainer> containerCls, String displayName) {
		var slc1 = getAPI().createNewX(containerCls);
		var slc2 = getAPI().createNewX(containerCls);
		...
		var pred2 = getAPI().newLocalVariableStatement(getAPI().newLocalVariable().withName("lv3").createNow());
		var st2 = this.cloneEObj(st1);
		var succ2 = this.cloneEObj(succ1);
	...
	@ParameterizedTest(name = "Statement in middle: {2}")
	@MethodSource("genTestParams")
	public void test_SamePredecessor_DifferentSuccessor(Class<? extends Statement> containeeCls,
			Class<? extends StatementListContainer> containerCls, String displayName) {
		var slc1 = getAPI().createNewX(containerCls);
		var slc2 = getAPI().createNewX(containerCls);
		...
		var pred2 = this.cloneEObj(pred1);
		var st2 = this.cloneEObj(st1);
		var succ2 = getAPI().newLocalVariableStatement(getAPI().newLocalVariable().withName("lv3").createNow());
	...
	@ParameterizedTest(name = "Statement in middle: {2}")
	@MethodSource("genTestParams")
	public void test_DifferentPredecessor_DifferentSuccessor(Class<? extends Statement> containeeCls,
			Class<? extends StatementListContainer> containerCls, String displayName) {
		var slc1 = getAPI().createNewX(containerCls);
		var slc2 = getAPI().createNewX(containerCls);
		 ...
		 * generated by containeeCls are similar, if similarity checking ignores their
