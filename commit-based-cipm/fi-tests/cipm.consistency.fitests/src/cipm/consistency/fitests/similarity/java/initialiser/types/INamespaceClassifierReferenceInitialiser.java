...
public interface INamespaceClassifierReferenceInitialiser
	...
	public default boolean setTarget(TypeReference tref, Classifier cls) {
		...
		if (cls != null) {
			 ...
			 */
			return result && castedTref.getNamespaces().size() == cls.getContainingContainerName().size()
					&& castedTref.getNamespaces().containsAll(cls.getContainingContainerName())
					&& castedTref.getClassifierReferences().size() == 1
					&& castedTref.getClassifierReferences().stream().anyMatch((cr) -> cr.getTarget().equals(cls));
		}
		return true;
	}
