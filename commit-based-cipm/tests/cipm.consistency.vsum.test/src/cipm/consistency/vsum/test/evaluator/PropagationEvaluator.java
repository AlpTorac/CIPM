...
/**
 * This class can be used to evaluate one propagation step, including the state
 * based change resolution.
 * 
 * This is achieved by comparing the model resulting from the propagation with
 * the parsed model of the target version.
 *
 * 
 * @param <CM> The used code model
 * @author Lukas Burgey
 */
public class PropagationEvaluator<CM extends ModelFacade> {
...
//            }
	...
	private boolean evaluateChangeResolutionTextFileSimilarity() {
		var targetModelPath = state.getCodeModelFacade().getDirLayout().getRootDirPath();
		var actualModelPath = state.getDirLayout().getVsumCodeModelPath();
	...
	private Resource getVsumCodeModel() {
		return getCodeModel(state.getDirLayout().getVsumCodeModelPath());
	}
	...
	private void evaluateCodeModelUpdate(EvaluationDataContainer eval) {
		...
		ResourceSet parseCodeModelResourceSet = state.getCodeModelFacade().getResourceSet();
		Resource parsedCodeModel = parseCodeModelResourceSet.getResources().get(0);
	...
	private Path locateStateCopyForCommit(Path directory, int commitCount, String commitId) {
		var currentName = propagation.getCommitIntegrationStateOriginalPath().getFileName();
		var lookupName = currentName + "-" + commitCount + "-" + commitId;
		var pathOfPossibleCommitCopy = directory.resolve(lookupName);
		if (pathOfPossibleCommitCopy.toFile().exists()) {
...
//            .getFileName();
	...
	private void evaluatePcmUpdateJaccardManual(EvaluationDataContainer eval) {
		...
		var referenceRepository = ModelUtil.readFromFile(
				stateCopyAutomatic.getPcmFacade().getDirLayout().getPcmRepositoryPath().toFile(), Repository.class);

		var newRepository = ModelUtil.readFromFile(state.getPcmFacade().getDirLayout().getPcmRepositoryPath().toFile(),
				Repository.class);
	...
	private void evaluatePcmUpdateJaccardAutomatic(EvaluationDataContainer eval) {
		...
		var stateCopyAutomatic = getCommitIntegrationStateForCommit(
				propagation.getCommitIntegrationStateCopyPath().getParent(), 1, propagation.getCommitId());
...
//            .toList();
	...
	private void evaluateImUpdate() {
		...
		var repo = state.getPcmFacade().getInMemoryPCM().getRepository();
		var im = state.getImFacade().getModel();
		var imEvalData = EvaluationDataContainer.get().getImUpdateEvalData();

		var repoFile = propagation.getPreviousPcmRepositoryPath().toFile();
		if (!repoFile.exists()) {
			return;
		}
...
//            evaluator.evaluateIMUpdate(repo, im, imEvalData, previousRepo);
	...
	public boolean evaluate() {
		...
		valid &= evaluateVsumRepositoryModel(state.getPcmFacade().getDirLayout().getPcmRepositoryPath());
		valid &= evaluateResultingImm(state.getImFacade().getDirLayout().getImFilePath());
