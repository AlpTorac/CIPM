...
public abstract class AbstractFITestResourceGenerator extends AbstractRepoTest {
	...
	/**
	 * Computes a unique suffix for the model to be generated based on
	 * the given parameter, so that generating multiple models is allowed.
	 * 
	...
	protected String getTestModelFileNameWOExtension(Object commitHashIdentifier) {
		return this.getTestModelFileNamePrefix() +
				this.getTestModelFileIdentifier(commitHashIdentifier);
	}
	...
	protected void initResGen(Object commitHashIdentifier) {
		...
		this.resGen.init(this.getTestType(), ".javaxmi");
	...
	protected void generateResourcesFor(Object commitHashIdentifier) throws Exception {
		...
		this.resGen.generateResourceWhile(() -> executePropagationAndEvaluation(null,
				this.getCommitHash(commitHashIdentifier), 0));
	}
}
